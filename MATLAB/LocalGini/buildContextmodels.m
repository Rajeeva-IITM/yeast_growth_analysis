...
if strcmp(MeM,'FASTCORE')
    ...
    for i=1:numel(contexts)
       ...
       model=sprintcore(cons_mod,current_core,tol); % Modified on 16-11-23 to work with sprintcore
...
elseif strcmp(MeM,'iMAT')
    ...
    for i=1:numel(contexts)
       ...
       model=iMAT(cons_mod,current_core,1,9,tol,{}, {}, 720);
       if exist('filename','var')
            % saving the model
            fn = strcat(filename,contexts{i});
            save(fn,'model');
...
elseif strcmp(MeM,'INIT')
    ...
    for i=1:numel(contexts)
       current_core = RxnImp(:,i);   
       model = INIT(cons_mod,current_core,tol, 60, 'INIT_Bloom2013_Log.txt');
       if exist('filename','var')
            % saving the model
            fn = strcat(filename,contexts{i});
            save(fn,'model');
